...
export class NodeLocalStore {
  ...
  constructor(nodeId: string) {
    const dir = path.join(os.homedir(), '.jackclaw', 'node', nodeId)
    fs.mkdirSync(dir, { recursive: true })
    this.dbPath = path.join(dir, 'messages.db')
  }
  ...
  async init(): Promise<boolean> {
    ...
    try {
      ...
      console.warn(`[node-local-store] init failed: ${(e as Error).message}`)
      return false
    }
  }
  ...
  save(msg: LocalMessage): void {
    ...
    try {
      this.db.run(
        `INSERT OR REPLACE INTO messages (id, thread_id, from_id, to_id, type, content, status, ts, encrypted, metadata)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [msg.id, msg.threadId ?? null, msg.from, msg.to, msg.type, msg.content,
  ...
  /** Get recent messages */
  recent(limit = 50): LocalMessage[] {
    if (!this.ready) return []
    const results = this.db.exec(
      'SELECT * FROM messages ORDER BY ts DESC LIMIT ?', [limit]
    )
    if (!results.length) return []
    return results[0].values.map((row: any[]) => this._rowToMsg(results[0].columns, row))
  }

  /** Get inbox (messages sent to this node) */
  inbox(toId: string, limit = 50): LocalMessage[] {
    if (!this.ready) return []
    const results = this.db.exec(
      'SELECT * FROM messages WHERE to_id = ? ORDER BY ts DESC LIMIT ?',
      [toId, limit]
    )
    if (!results.length) return []
    return results[0].values.map((row: any[]) => this._rowToMsg(results[0].columns, row))
  ...
  /** Get thread messages */
  thread(threadId: string, limit = 100): LocalMessage[] {
    if (!this.ready) return []
    const results = this.db.exec(
      'SELECT * FROM messages WHERE thread_id = ? ORDER BY ts ASC LIMIT ?',
      [threadId, limit]
    )
    if (!results.length) return []
    return results[0].values.map((row: any[]) => this._rowToMsg(results[0].columns, row))
  ...
  /** Stats */
  stats(): { total: number; threads: number } {
    if (!this.ready) return { total: 0, threads: 0 }
  ...
  private _rowToMsg(columns: string[], values: any[]): LocalMessage {
    const obj: any = {}
