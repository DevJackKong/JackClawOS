...
const task_state_store_1 = require("../store/task-state-store");
const server_1 = require("../server");
const router = (0, express_1.Router)();
const store = task_state_store_1.taskStateStore;
/**
 ...
 * 统一处理“资源不存在”响应。
 */
function sendNotFound(res, message = 'task_not_found') {
...
router.post('/', (0, server_1.asyncHandler)(async (req, res) => {
    ...
    const tenantId = (payload.tenantId || req.tenantContext?.tenantId || '').trim();
    const title = (payload.title || '').trim();
    const creatorId = (payload.creatorId || req.tenantContext?.userId || '').trim();
    if (!tenantId) {
    ...
    const task = await store.create(tenantId, title, creatorId, payload);
 ...
 * - actorId?: string
 */
router.post('/:id/transition', (0, server_1.asyncHandler)(async (req, res) => {
    const { event, actorId } = (req.body ?? {});
 ...
 * - actorId?: string
 */
router.post('/:id/assign', (0, server_1.asyncHandler)(async (req, res) => {
 ...
 * 获取任务状态变更历史。
 */
router.get('/:id/history', (0, server_1.asyncHandler)(async (req, res) => {
    const task = await store.get(req.params.id);
    if (!task) {
        sendNotFound(res);
        return;
