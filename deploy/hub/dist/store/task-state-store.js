...
exports.taskStateStore = exports.TaskStateStore = void 0;
...
const TASKS_STATE_FILE = path_1.default.join(HUB_DIR, 'tasks-state.json');
/**
...
class TaskStateStore {
     ...
     */
    load() {
        const data = loadJSON(TASKS_STATE_FILE, { tasks: {} });
     ...
     */
    create(tenantId, title, creatorId, opts = {}) {
        const tasks = this.load();
        const now = Date.now();
        const task = {
            id: opts.id ?? crypto_1.default.randomUUID(),
            tenantId,
     ...
     */
    transition(id, event, actorId) {
        const tasks = this.load();
        const task = tasks[id];
        if (!task)
            throw new Error(`Task not found: ${id}`);
        const nextState = this.resolveNextState(task.state, event, task);
        const now = Date.now();
     ...
     */
    resolveNextState(currentState, event, context) {
        const machine = new state_machine_1.StateMachine(state_machine_1.TaskStateMachine, context);
        void machine;
        const transition = state_machine_1.TaskStateMachine.transitions.find((item) => {
...
exports.taskStateStore = new TaskStateStore();
