...
const COLLABS_FILE = path_1.default.join(HUB_DIR, 'collaborations.json');
const TRUST_FILE = path_1.default.join(HUB_DIR, 'trust.json');
...
let collaborations = loadJSON(COLLABS_FILE, {});
let trustGraph = loadJSON(TRUST_FILE, {});
// ─── Helpers ──────────────────────────────────────────────────────────────────
function trustKey(from, to) { return `${from}→${to}`; }
function getTrust(from, to) {
    return trustGraph[trustKey(from, to)] ?? null;
}
function setTrust(rel) {
    trustGraph[trustKey(rel.fromHandle, rel.toHandle)] = rel;
    saveJSON(TRUST_FILE, trustGraph);
}
function shouldAutoAccept(fromHandle, toHandle) {
...
router.post('/register', (req, res) => {
    ...
    const profile = {
        ...
        hubUrl: req.hubUrl ?? `http://localhost:${process.env.HUB_PORT ?? 3100}`,
...
router.get('/lookup/:handle', (req, res) => {
    ...
    const parsed = (0, protocol_1.parseHandle)(raw);
    if (!parsed)
        return res.status(400).json({ error: 'invalid_handle' });
    const profile = directory_1.directoryStore.getProfile(parsed.full);
    if (!profile) {
        return res.json({ found: false, handle: parsed.full, isLocal: true });
    }
    directory_1.directoryStore.touchHandle(parsed.full);
...
router.get('/list', (_req, res) => {
    const visible = directory_1.directoryStore.listPublic();
...
router.post('/collab/invite', (req, res) => {
    ...
    const invite = {
        ...
        toHandle: targets.map(t => (0, protocol_1.parseHandle)(t).full).join(', '),
        sessionId,
        topic: body.topic,
    ...
    const autoAccepted = targets.filter(t => {
        const tp = (0, protocol_1.parseHandle)(t).full;
    ...
    const session = {
        sessionId,
        inviteId,
        participants: [fromParsed.full, ...targets.map(t => (0, protocol_1.parseHandle)(t).full)],
...
router.post('/collab/respond', (req, res) => {
    ...
    const newLevel = body.decision === 'decline' ? 'contact'
...
router.patch('/collab/sessions/:sessionId', (req, res) => {
...
router.get('/collab/trust/:fromHandle/:toHandle', (req, res) => {
    const from = (0, protocol_1.parseHandle)(decodeURIComponent(req.params.fromHandle));
    const to = (0, protocol_1.parseHandle)(decodeURIComponent(req.params.toHandle));
    if (!from || !to)
        return res.status(400).json({ error: 'invalid_handle' });
