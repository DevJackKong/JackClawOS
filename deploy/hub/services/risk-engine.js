...
exports.initDefaultRules = initDefaultRules;
...
function getRecommendationsForRule(rule) {
    switch (rule.id) {
...
class RiskEngine {
    ...
    evaluate(ctx) {
        ...
        const level = triggeredRules.reduce((current, rule) => (RISK_LEVEL_PRIORITY[rule.level] > RISK_LEVEL_PRIORITY[current] ? rule.level : current), 'low');
     ...
     * 检测非工作时间写操作。
     */
    isOffHoursWriteOperation(action, metadata, timestamp) {
 ...
 * 初始化全部默认风控规则。
 */
function initDefaultRules(engine = exports.riskEngine) {
    engine.addRule({
...
initDefaultRules(exports.riskEngine);
