...
// ─── 共享技能存储（Hub 级别）──────────────────────────────────────────────────
const HUB_SKILLS_PATH = path_1.default.join(os_1.default.homedir(), '.jackclaw', 'hub-skills.json');
function loadSharedSkills() {
    if (!fs_1.default.existsSync(HUB_SKILLS_PATH))
        return [];
    try {
        return JSON.parse(fs_1.default.readFileSync(HUB_SKILLS_PATH, 'utf-8'));
    }
    catch {
        return [];
    }
}
function saveSharedSkills(skills) {
    fs_1.default.writeFileSync(HUB_SKILLS_PATH, JSON.stringify(skills, null, 2));
}
// ─── Skill Routes ────────────────────────────────────────────────────────────
/**
...
router.get('/skills/stats', (_req, res) => {
    ...
    res.json({
        ...
        topContributors: Array.from(skills.reduce((m, s) => m.set(s.sharedBy, (m.get(s.sharedBy) || 0) + 1), new Map()))
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
 ...
 */
router.post('/skills/share', (req, res) => {
...
// ─── Reflexion Routes ────────────────────────────────────────────────────────
const REFLEXION_DIR = path_1.default.join(os_1.default.homedir(), '.jackclaw', 'reflexion');
/**
...
router.get('/reflexion', (req, res) => {
    ...
    if (!rawNodeId) {
        // 列出所有节点
        if (!fs_1.default.existsSync(REFLEXION_DIR)) {
            res.json({ nodes: [] });
            return;
        }
        const nodes = fs_1.default.readdirSync(REFLEXION_DIR).filter(f => fs_1.default.statSync(path_1.default.join(REFLEXION_DIR, f)).isDirectory());
        res.json({ nodes });
        return;
    }
    const nodeId = sanitizeNodeId(rawNodeId);
    ...
    const filePath = path_1.default.join(REFLEXION_DIR, nodeId, 'reflexions.json');
    if (!fs_1.default.existsSync(filePath)) {
        res.json({ nodeId, total: 0, entries: [] });
        return;
    }
    try {
        const entries = JSON.parse(fs_1.default.readFileSync(filePath, 'utf-8'));
        res.json({
            nodeId,
 ...
 */
router.get('/reflexion/stats', (req, res) => {
    const rawNodeId = String(req.query.nodeId || '');
    if (!rawNodeId) {
