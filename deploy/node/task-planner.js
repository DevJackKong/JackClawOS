...
class TaskPlanner {
    ...
    async plan(opts) {
        ...
        try {
            ...
            console.warn('[planner] AI refinement failed, using heuristic plan:', e.message);
        }
    ...
    heuristicPlan(opts) {
        ...
        const parallelMinutes = batches.reduce((s, batch) => s + Math.max(...batch.map(id => subtasks.find(t => t.id === id).estimatedMinutes)), 0);
    ...
    // ─── AI 细化规划 ────────────────────────────────────────────────────────────
    async aiRefinePlan(opts, basePlan) {
...
请输出以下 JSON 结构（严格遵循，不要增减字段）：
{
  ...
  "risks": ["风险1", "风险2"]
        ...
        // 合并 AI 结果和基础计划
        const subtasks = (aiData.subtasks ?? []).map((s, i) => ({
            ...s,
            id: s.id || `s${i + 1}`,
            assignTo: undefined,
        }));
    ...
    // ─── 工具函数 ───────────────────────────────────────────────────────────────
    guessComplexity(desc, wordCount) {
        if (wordCount < 20)
            return 'trivial';
        ...
        if (complexKeywords.some(k => desc.includes(k)) || wordCount > 200)
            return 'complex';
        if (moderateKeywords.some(k => desc.includes(k)) || wordCount > 80)
            return 'moderate';
        if (simpleKeywords.some(k => desc.includes(k)) || wordCount > 30)
            return 'simple';
        return 'trivial';
    }
    complexityToMinutes(c) {
        return { trivial: 5, simple: 20, moderate: 60, complex: 180, epic: 480 }[c];
    }
    extractSubtasks(taskId, description, complexity) {
        // 简单启发：按段落/数字列表切分
    ...
    estimateCost(tokens) {
        ...
        return Math.round((inputTokens / 1_000_000 * USD_PER_1M_INPUT + outputTokens / 1_000_000 * USD_PER_1M_OUTPUT) * 100) / 100;
    }
}
